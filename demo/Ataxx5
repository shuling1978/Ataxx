...
{
    int wins=0;
    unsigned long long originalBlackGrid = blackGrid;
    unsigned long long originalWhiteGrid = whiteGrid;
    int originalBlackCount = blackPieceCount, originalWhiteCount = whitePieceCount;
//存储原来的棋盘和黑白棋子count
    ...
    for (int i = 0; i < simulations; i++)
    {
        ...
        //开始使用sim头
        unsigned long long simBlackGrid = blackGrid;
        unsigned long long simWhiteGrid = whiteGrid;
        ...
        while (!gameOver && steps < maxDepth)
        {
            ...
            for (int y0 = 0; y0 < 7; y0++)
            {
                for (int x0 = 0; x0 < 7; x0++)
                {
                    if ((currentColor == 1 && (simBlackGrid & (1ULL << (x0 * 7 + y0))) != 0) ||
                        (currentColor == -1 && (simWhiteGrid & (1ULL << (x0 * 7 + y0))) != 0))//颜色相同，可以移动这个棋子
                   {
                     for (int dir = 0; dir < 24; dir++)
                        {
                            int x1 = x0 + delta[dir][0];
                            int y1 = y0 + delta[dir][1];
                            if(inMap(x1,y1)&&(!((simBlackGrid | simWhiteGrid) & (1ULL << (x1 * 7 + y1))))){
                                //int tempGrid[7][7];
                                unsigned long long tempBlackGrid = simBlackGrid, tempWhiteGrid = simWhiteGrid;
                                int tempBlackCount = simBlackCount, tempWhiteCount = simWhiteCount;
                                ...
                                if (dx == 2 || dy == 2) {
                                    // 如果是跳跃操作，移除起始位置的棋子
                                    if (currentColor == 1) {
                                        tempBlackGrid &= ~(1ULL << (x0 * 7 + y0)); // 从黑棋位图中移除起始位置的棋子
                                    } else {
                                        tempWhiteGrid &= ~(1ULL << (x0 * 7 + y0)); // 从白棋位图中移除起始位置的棋子
                                    }
                                } else if (currentColor == 1) {
                                    tempBlackCount++; // 如果是复制操作，增加黑棋数量
                                } else {
                                    tempWhiteCount++; // 如果是复制操作，增加白棋数量
                                }
                                
                                    // 在目标位置放置棋子
                                if (currentColor == 1)
                                tempBlackGrid |= 1ULL << (x1 * 7 + y1);
                            else
                                tempWhiteGrid |= 1ULL << (x1 * 7 + y1);
                                ...
                                for (int dir2 = 0; dir2 < 8; dir2++)
                                {
                                    ...
                                    if (!inMap(x, y))
                                        continue;
                                        if(currentColor == 1 && (tempWhiteGrid & (1ULL << (x * 7 + y))) != 0)
                                        {
                                            currCount++;
                                            tempWhiteGrid &= ~(1ULL << (x * 7 + y));
                                            tempBlackGrid |= 1ULL << (x * 7 + y);
                                        }
                                        else if(currentColor == -1 && (tempBlackGrid & (1ULL << (x * 7 + y))) != 0)
                                        {
                                            currCount++;
                                            tempBlackGrid &= ~(1ULL << (x * 7 + y));
                                            tempWhiteGrid |= 1ULL << (x * 7 + y);
            ...
            if (dx == 2 || dy == 2)
                simBlackGrid &= ~(1ULL << (move.first.first * 7 + move.first.second));
            else if (currentColor == 1)
                simBlackCount++;
            else
                simWhiteCount++;
            ...
            if (currentColor == 1)
                simBlackGrid |= 1ULL << (move.second.first * 7 + move.second.second);
            else
                simWhiteGrid |= 1ULL << (move.second.first * 7 + move.second.second);
            int captureCount = 0;
            ...
            for (int adjDir = 0; adjDir < 8; adjDir++)
            {
                ...
                if (!inMap(x, y))
                    continue;
                if (currentColor == 1 && (simWhiteGrid & (1ULL << (x * 7 + y))) != 0)
                {
                    captureCount++;
                    simWhiteGrid &= ~(1ULL << (x * 7 + y));
                    simBlackGrid |= 1ULL << (x * 7 + y);
                }
                else if (currentColor == -1 && (simBlackGrid & (1ULL << (x * 7 + y))) != 0)
                {
                    captureCount++;
                    simBlackGrid &= ~(1ULL << (x * 7 + y));
                    simWhiteGrid |= 1ULL << (x * 7 + y);
   ...
   // blackPieceCount = originalBlack;
    ...
    blackGrid = originalBlackGrid;
    whiteGrid = originalWhiteGrid;
    blackPieceCount = originalBlackCount;
    whitePieceCount = originalWhiteCount;
